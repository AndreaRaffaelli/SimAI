...
void send_finish(FILE *fout, Ptr<RdmaQueuePair> q)
{
  ...
  if (logLevel == NcclLogLevel::DEBUG)
  {
    {
      fprintf(fout, "[Packet sent from NIC] send finish, src:  %d did:  %d port:  %d srcip  %d dstip  %d total bytes:  %llu at the tick:  %d\n", sid, did, q->sport, q->sip, q->dip, q->m_size, AstraSim::Sys::boostedTick());
      fflush(fout);
    }
